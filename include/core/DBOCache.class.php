...
class DBOCache{
    ...
    {
		$this->config['DATA_CACHE_PATH'] = isset($config['DATA_CACHE_PATH'])?$config['DATA_CACHE_PATH']:'./data/db_cache/';	//缓存目录
		...
		$this->config['DATA_CACHE_FILE']=isset($config['DATA_CACHE_FILE'])?$config['DATA_CACHE_FILE']:'cachedata';//缓存的数据文件名
		$this->config['DATA_CACHE_SIZE']=isset($config['DATA_CACHE_SIZE'])?$config['DATA_CACHE_SIZE']:'15M';//预设的缓存大小
		$this->config['DATA_CACHE_FLOCK']=isset($config['DATA_CACHE_FLOCK'])?$config['DATA_CACHE_FLOCK']:true;//是否存在文件锁，设置为false，将模拟文件锁
...
*/
    ...
    public function set($name,$value,$expire=NULL)
    {
        ...
        return $this ->store($key,$data);//存储数据
    }
    ...
    private function _format($truncate=false){
        if($this->lock(true,true)){
            ...
            $this->max_size = $this->_parse_str_size($this->config['DATA_CACHE_SIZE'],15728640); //default:15m
