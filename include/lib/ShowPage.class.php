...
class ShowPage{
	...
	public function show($url,$total,$pagesize=10,$pageBarNum=5,$mode=1){
		if($total > $pagesize){
			$pagesize = $pagesize ? $pagesize : 10;
			$pageBarNum = $pageBarNum ? $pageBarNum : 5;
			...
			switch($mode){
				case '1':
					...
					break;
				case '2':
					return $this->total.'&nbsp;'.$total.'&nbsp;'.$this->pnum.'&nbsp;&nbsp;'.$this->perPage.'&nbsp;'.$pagesize.'&nbsp;'.$this->pnum.'&nbsp;&nbsp;'.$this->prePage().$this->nowBar().$this->nextPage().$this->select();
					break;
			}
		}	
	}
